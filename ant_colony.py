...
class CVPR:
    def __init__(
        self, graph: np.array, demande: list[int], n_vehicules: int, capacity: int
    ):
        self.graph = graph
    ...
    def ant_colony_optimization(
        ...
        for iteration in range(n_iterations):
            ...
            for ant in range(n_ants):
                ...
                for i in range(self.n_vehicules):
                    ...
                    while False in visited:  # while not all nodes are visited
                        ...
                        for candidate in range(self.n_points):
                            if (
                                (car_capacities[i] - self.demande[candidate] >= 0)
                                and candidate != car_current_points[i]
                                and not visited[candidate]
                            ):
                                has_unvisited = True
                                probabilities[i][candidate] = (
                                    self.pheromone[car_current_points[i], candidate]
                                    ** alpha
                                    / self.graph[car_current_points[i], candidate]
                                    ** beta
                                )
                            else:
                ...
                for i, car_path in enumerate(car_paths):
                    ...
                    car_path.append(
                        (car_path_lengths[i], self.capacity - car_capacities[i])
                    )
                ...
                print(
                    f"Iteration {iteration+1}: Ant:{ant+1}/{n_iterations} - Best path length: {best_solution_length} - current path length: {path_length}"
                )
            ...
            for solution, solution_length in zip(solutions, solution_lengths):
                for sub_path in solution:
                    for i in range(len(sub_path) - 2):
                        self.pheromone[sub_path[i], sub_path[i + 1]] += (
                            Q / solution_length
                        )
        ...
        return result, best_solution_length
